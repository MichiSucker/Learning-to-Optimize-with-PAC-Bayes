...
class TestTrainingAssistant(unittest.TestCase):
    ...
    def test_get_bins(self):
        self.assertEqual(self.training_assistant.get_bins(), [1e0, 1e-4, 1e-8, 1e-12, 1e-16, 1e-20, 1e-24, 1e-28][::-1])

    def test_set_bins(self):
        new_bins = [1, 2, 3]
        old_bins = self.training_assistant.get_bins()
        self.assertNotEqual(old_bins, new_bins)
        self.training_assistant.set_bins(new_bins=new_bins)
        self.assertEqual(self.training_assistant.get_bins(), new_bins)
