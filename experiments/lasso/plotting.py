...
def create_evaluation_plots(loading_path: str, path_of_experiment: str) -> None:
    ...
    names = {'std': 'FISTA', 'pac': 'Learned', 'other': 'other'}
    colors = {'std': '#FA3453', 'pac': '#00D3FF', 'other': '#006a78'}
    ...
    assert number_of_iterations_for_approximation == 5000
    ...
    ax[0, 0].axhline(mean_opt, 0, 1, color=colors['other'], linestyle='dashed',
                     label='$\ell(x_{\\rm{FISTA}}^{(5000)}, \\theta)$')
    ...
    bins = np.logspace(np.log10(np.minimum(np.min(losses_of_learned_algorithm[:, n_train + 1]),
                                           np.min(losses_of_baseline_algorithm[:, n_train + 1]))),
                       np.log10(np.maximum(np.max(losses_of_learned_algorithm[:, n_train + 1]),
                                           np.max(losses_of_baseline_algorithm[:, n_train + 1]))), 50)
