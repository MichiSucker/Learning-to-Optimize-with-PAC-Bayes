...
def create_evaluation_plots(loading_path: str, path_of_experiment: str) -> None:
    ...
    colors = {'std': '#870000', 'pac': '#00D3FF', 'other': '#006a78'}

    losses_of_adam = np.load(loading_path + 'losses_of_adam.npy')
    losses_of_learned_algorithm = np.load(loading_path + 'losses_of_learned_algorithm.npy')
    classification_rate_adam = np.load(loading_path + 'classification_rate_adam.npy')
    classification_rate_of_learned_algorithm = np.load(loading_path + 'classification_rate_of_learned_algorithm.npy')
    # For plotting.
    eps = 1e-12
    losses_of_adam[losses_of_adam <= eps] = eps
    losses_of_learned_algorithm[losses_of_learned_algorithm <= eps] = eps
    # ground_truth_losses = np.load(loading_path + 'ground_truth_losses.npy')
    ...
    with open(loading_path + 'times_of_adam', 'rb') as file:
        times_of_adam = pickle.load(file)

    with open(loading_path + 'parameters_of_estimation', 'rb') as file:
        parameters_of_estimation = pickle.load(file)

    levels_of_accuracy = list(times_of_adam.keys())
    ...
    mean_std, mean_pac = np.mean(losses_of_adam, axis=0), np.mean(losses_of_learned_algorithm, axis=0)
    median_std, median_pac = np.median(losses_of_adam, axis=0), np.median(losses_of_learned_algorithm, axis=0)
    iterations = np.arange(0, losses_of_adam.shape[1])
    ...
    ax[0, 0].fill_between(iterations, np.quantile(losses_of_adam, q=0.1, axis=0),
                          np.quantile(losses_of_adam, q=0.9, axis=0),
    ...
    # Plot classification rates into the same plot
    ax_2 = ax[0, 0].twinx()

    mean_rate_std = np.mean(classification_rate_adam, axis=0)
    mean_rate_pac = np.mean(classification_rate_of_learned_algorithm, axis=0)

    # Plot rates
    ax_2.plot(iterations, mean_rate_std, color=colors['std'], alpha=0.5, linestyle=(0, (3, 1, 1, 1, 1, 1)))
    ax_2.plot(iterations, mean_rate_pac, color=colors['pac'], alpha=0.5, linestyle=(0, (3, 1, 1, 1, 1, 1)))
    ax_2.set_ylabel('Success rate [\\%]')
    ax_2.set_ylim([0,100])
    ...
    ax[1, 0].axvline(np.median(losses_of_learned_algorithm[:, n_train + 1]), color=colors['pac'], linestyle='dotted',
                     alpha=0.5)
    ax[1, 0].axvline(np.mean(losses_of_adam[:, n_train + 1]), color=colors['std'], linestyle='dashed', alpha=0.5)
    ax[1, 0].axvline(np.median(losses_of_adam[:, n_train + 1]), color=colors['std'], linestyle='dotted', alpha=0.5)
    ...
    for epsilon, linestyle in zip(levels_of_accuracy, linestyles):
        ...
        total_time_pac, total_time_std = (np.cumsum(times_of_learned_algorithm[epsilon]),
                                          np.cumsum(times_of_adam[epsilon]))
    ...
    # a and b for posterior regard to an uninformative prior
    a = 1 + success
    b = 1 + failure
