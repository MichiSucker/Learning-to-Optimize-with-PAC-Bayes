...
def create_evaluation_plots(loading_path: str, path_of_experiment: str) -> None:
    ...
    if ground_truth_losses is not None:

        # Compute also the same statistics for the losses at the optimum
        mean_opt = np.mean(ground_truth_losses)
        median_opt = np.median(ground_truth_losses)

        # Plot optimal loss
        ax[0, 0].axhline(mean_opt, 0, 1, color=colors['other'], linestyle='dashed', label='$c(g(x), y_{\\rm{obs}})$')
