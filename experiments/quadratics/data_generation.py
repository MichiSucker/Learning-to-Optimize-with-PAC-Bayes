...
def get_distribution_of_strong_convexity_parameter():
...
def get_distribution_of_smoothness_parameter():
...
def create_parameter(diagonal, right_hand_side):
...
def get_values_of_diagonal(min_value, max_value, number_of_values):
...
def get_parameters(number_of_datapoints_per_dataset):
    ...
    parameters = {}
    for number_of_functions, name in [(n_prior, 'prior'), (n_train, 'train'), (n_test, 'test'), (n_validation, 'validation')]:
