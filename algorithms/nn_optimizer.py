        ...
        coefficients = self.coefficients(
            torch.concat(
                (torch.log(1 + grad_norm.reshape((1,))),
                 ...
                 torch.log(opt_algo.loss_function(opt_algo.current_state[1]).detach().reshape((1,))),
                 torch.log(opt_algo.loss_function(opt_algo.current_state[0]).detach().reshape((1,)))))
        )
        direction = self.update_layer(torch.concat((coefficients[0] * self.gradient * grad.reshape((1, 1, 1, -1)),
                                                    ...
                                                    opt_algo.current_state[0].reshape((1, 1, 1, -1)),
